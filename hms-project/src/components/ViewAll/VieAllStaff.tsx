...
const ViewAllStaff: React.FC = () => {
  ...
  const validateForm = (): string[] => {
    ...
    if (!staffData.firstName.trim()) errorsList.push("First Name is required.");
    if (!staffData.lastName.trim()) errorsList.push("Last Name is required.");
    if (staffData.phoneNumber && !/^\d{10}$/.test(staffData.phoneNumber)) {
      errorsList.push("Phone Number must be a valid 10-digit number.");
    }
    if (staffData.email && !/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/.test(staffData.email)) {
      errorsList.push("Email must be a valid email address.");
    }
    return errorsList;
  };
